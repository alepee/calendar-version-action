...
import { ValidationError } from '../validateInputs';
...
class ConfigLoader {
    static async loadConfig(): Promise<CalverConfig> {
        ...
        try {
            ...
            throw new ValidationError(`Failed to load config: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
...
export { ConfigLoader, CalverConfig }; 
